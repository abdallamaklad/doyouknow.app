...
- **"Latest Articles" is not latest.** Rendered dates run 2025-01-20 → 2025-01-20 → 2026-06-26 → 2026-06-15, in strict alphabetical slug order. **Cause (corrected):** the homepage listing is not generated at all — `en/index.html` and `ar/index.html` are hand-authored *source*, and `prepare.mjs` only post-processes them (it generates the category tiles, not the article rows). So there is no sort to be wrong; the rows were simply written out alphabetically. The RSS/JSON feeds *do* sort newest-first (`prepare.mjs:1502-1503`), so the page contradicts its own feed. *(An earlier revision of this report cited `prepare.mjs:1265` as the sort — that line orders the sitemap and is unrelated.)*
